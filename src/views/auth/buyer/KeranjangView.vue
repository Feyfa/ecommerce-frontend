<template>
    ...
    >
        <div class="w-full bg-slate-50 px-3 pb-28 pt-4 sm:px-5 lg:px-6 lg:pb-6">
            ...
            <div
                class="keranjang-container flex flex-col items-start gap-5 lg:flex-row"
            >
                <div
                ...
                >
                    ...
                    >
                        <div
                            class="flex max-w-2xl flex-col items-center gap-5 text-center sm:flex-row sm:text-left"
                        >
                            <div
                                class="flex h-24 w-24 shrink-0 items-center justify-center rounded-full bg-violet-50 text-violet-500"
                            >
                                <i
                                    class="fa-solid fa-cart-shopping text-4xl"
                                ></i>
                            </div>
                            ...
                            >
                                ...
                                >
                                    Yuk, isi dengan produk yang kamu butuhkan.
                                    Produk yang kamu pilih nanti akan muncul di
                                    sini sebelum checkout.
                                </p>
                    ...
                    >
                        <label
                            class="flex cursor-pointer items-center gap-3 text-sm font-semibold text-slate-900"
                        >
                            <input
                                @change="checkedKeranjangAll"
                                :checked="isCheckedKeranjangAll()"
                                :disabled="
                                    availableKeranjangCount === 0 ||
                                    isProcessCheckout ||
                                    isProcessChecked
                                "
                                type="checkbox"
                        ...
                        <span class="text-sm font-medium text-slate-500"
                            >{{ selectedKeranjangCount }} dari
                            {{ availableKeranjangCount }} produk dipilih</span
                        >
                    </div>
                    ...
                    >
                        <div class="flex items-start gap-3">
                            <i
                                class="fa-solid fa-triangle-exclamation mt-0.5 text-amber-600"
                            ></i>
                            <div>
                                <p class="text-sm font-semibold">
                                    {{ cartReviewCount }} produk perlu diperiksa
                                </p>
                                <p
                                    class="mt-1 text-xs leading-5 text-amber-800"
                                >
                                    Stok produk berubah sejak ditambahkan ke
                                    keranjang. Periksa produk yang ditandai
                                    sebelum checkout.
                                </p>
                            </div>
                        </div>
                    ...
                    >
                        ...
                        >
                            <input
                                @change="
                                    checkedKeranjangGroup(
                                        $event,
                                        keranjang[0].k_user_id_seller,
                                    )
                                "
                            ...
                            <div class="flex min-w-0 flex-col">
                                <span
                                    ...
                                    >{{ keranjang[0].u_seller_name }}</span
                                >
                                <span class="text-xs font-medium text-slate-500"
                                    >Pilih semua dari toko ini</span
                                >
                            </div>
                        </div>
                            ...
                            >
                                <div class="min-w-0">
                                    ...
                                    >
                                        ...
                                        >, tetapi jumlah Anda
                                        <strong>{{
                                            item.stock_issue.cart_quantity
                                        }}</strong
                                ...
                                <div
                                    class="flex shrink-0 items-center justify-between gap-3 sm:flex-col sm:items-end"
                                >
                                    <button
                                        type="button"
                                    ...
                                    >
                                        <i
                                            class="fa-regular fa-trash-can text-sm"
                                        ></i>
                                    </button>
                                    ...
                                    >
                                        ...
                                        <input
                                            ...
                                            "
                                            @input="
                                                validationTotalKeranjang(
                                                    $event,
                                                    item,
                                                )
                                            "
                                            @blur="
                                                changeTotalKeranjang(
                                                    item.p_id,
                                                    item,
                                                )
                                            "
                ...
                >
                    <div
                        class="min-w-0 flex-1 lg:border-b lg:border-b-slate-200 lg:pb-3"
                    >
                        <h2
                            class="hidden text-base font-semibold text-slate-950 lg:block"
                        >
                            Ringkasan Belanja
                        </h2>
                        <div
...
export default {
    ...
    computed: {
        keranjangGroups() {
            return Object.values(this.keranjangs || {}).filter(
        ...
        availableKeranjangCount() {
            return this.keranjangGroups
                .flat()
                .filter((item) => this.isItemSelectable(item)).length;
        },
            ...
            return this.keranjangGroups
                ...
                .filter(
                    (item) =>
                        ...
                        (item.k_checked === 1 || item.k_checked === true),
                ).length;
        },
        ...
        cartReviewCount() {
            return this.keranjangGroups
                .flat()
                .filter((item) => this.hasCartReviewIssue(item)).length;
        },
    ...
    methods: {
        ...
        hasQuantityStockIssue(item) {
            return (
                item?.stock_issue?.code === "QUANTITY_EXCEEDS_STOCK" &&
                Number(item.stock_issue.available_stock) > 0
            );
        },

        hasCartReviewIssue(item) {
            return (
                this.hasQuantityStockIssue(item) ||
                item?.unavailable_reason === "OUT_OF_STOCK"
            );
        },
        ...
        updateLocalTotalPrice(item, quantityDelta) {
            if (this.isCheckedItem(item)) {
                this.totalPrice =
                    ...
                    Number(item.p_price || 0) * quantityDelta;
            }
        },
        ...
        handleBuyerAddressRequired(responseData) {
            ...
            ElMessageBox.alert(
                responseData.message ||
                    "Tambahkan alamat pengiriman sebelum melanjutkan checkout.",
        ...
        scrollToFirstCartReviewIssue(issues = []) {
            const firstCartId =
                ...
                this.keranjangGroups
                    .flat()
                    .find((item) => this.hasCartReviewIssue(item))?.k_id;
            ...
            this.$nextTick(() => {
                document
                    .getElementById(`cart-item-${firstCartId}`)
                    ?.scrollIntoView({
                        behavior: "smooth",
                        block: "center",
                    });
        ...
        checkout() {
            ...
            const keranjangAlReadyChecked = Object.values(this.keranjangs).some(
                (group) =>
                    group.some(
                        (item) =>
                            this.isItemSelectable(item) &&
                ...
                this.$store
                    ...
                    .catch((error) => {
                        ...
                        if (responseData?.code === "CART_STOCK_CHANGED") {
                            ...
                            this.scrollToFirstCartReviewIssue(
                                responseData.issues || [],
                            );
                            return;
                        }
        ...
        changeTotalKeranjang(product_id, item) {
            const previousTotal = Number(
                this.quantityBeforeEdit[product_id] || item.k_total || 1,
            );
        ...
        adjustQuantityToStock(item) {
            const availableStock = Number(
                item?.stock_issue?.available_stock || 0,
            );
        ...
        isCheckedKeranjangGroup(keranjang) {
            const availableItems = keranjang.filter((item) =>
                this.isItemSelectable(item),
            );
            ...
            return (
                ...
                availableItems.every(
                    (item) => item.k_checked === 1 || item.k_checked === true,
                )
            );
        },
        ...
        isCheckedKeranjangAll() {
            return (
                this.availableKeranjangCount > 0 &&
                this.selectedKeranjangCount === this.availableKeranjangCount
            );
        },
    },
};
