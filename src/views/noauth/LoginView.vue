<template>
  <div 
    ...
    <FormOtpView v-model:show="show.formOtpView" :otpSecretKey="otpSecretKey" :email="email" :password="password" />
    ...
    <form 
      ...
      <div class="flex justify-end mt-3">
          <router-link to="/register" class="underline text-blue-700">register</router-link>
      </div>

      <button 
        type="button" 
        class="w-full h-12 border border-neutral-300 rounded shadow bg-blue-500 mt-4"
        :class="isProcessLogin ? 'opacity-85' : 'hover:bg-[#428bff]'"
        :disabled="isProcessLogin"
        @click="loginSubmit">
        Login
        <i v-if="isProcessLogin" class="ml-1 fas fa-spinner fa-pulse"></i>
      </button>
    </form>
  </div>
</template>
