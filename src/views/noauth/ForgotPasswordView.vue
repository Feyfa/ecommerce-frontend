                ...
                <div class="auth-form">
                    ...
                    <template v-else-if="isVerifyCodeStep">
                        <div class="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700">
                            Kode reset password sudah dikirim ke {{ email }}. Cek inbox email Anda lalu masukkan kode
                            terbaru.
                        </div>
                    ...
                    <template v-else>
                        ...
                        <div class="auth-field">
                            ...
                            <div class="auth-input-wrap">
                                ...
                                <button
                                    ...
                                    :aria-label="
                                        isShowPassword ? 'Sembunyikan password baru' : 'Tampilkan password baru'
                                    "
                        ...
                        <div class="auth-field">
                            ...
                            <div class="auth-input-wrap">
                                ...
                                >
                                    <i
                                        :class="isShowConfirmPassword ? 'fa-regular fa-eye-slash' : 'fa-regular fa-eye'"
                                    ></i>
                                </button>
                            </div>
                            <small v-if="errors.confirmPassword" class="auth-error-text">{{
...
import { getClerkErrorMessage, getClerkRuntimeState, isClerkEnabled, waitForClerkLoaded } from '@/clerk';
...
export default {
    /**
     * Menginisialisasi state Composition API dan integrasi komponen pada halaman lupa password.
     *
    ...
    /**
     * Membuat state reaktif yang digunakan komponen untuk halaman forgot password.
     *
    ...
    computed: {
        /**
         * Menentukan apakah kondisi clerk enabled terpenuhi untuk halaman forgot password.
         *
        ...
        /**
         * Menentukan apakah kondisi request code step terpenuhi untuk halaman forgot password.
         *
         * @returns {boolean} Menunjukkan apakah kondisi is request code step terpenuhi.
         */
        isRequestCodeStep() {
        ...
        /**
         * Menentukan apakah kondisi verify code step terpenuhi untuk halaman forgot password.
         *
         * @returns {boolean} Menunjukkan apakah kondisi is verify code step terpenuhi.
         */
        isVerifyCodeStep() {
        ...
        /**
         * Mengembalikan form title ke state awal untuk halaman forgot password.
         *
         * @returns {string} Teks reset form title yang telah diformat atau ditentukan.
         */
        resetFormTitle() {
            if (this.isVerifyCodeStep) return 'Verifikasi Kode';

            if (this.resetStep === resetPasswordSteps.newPassword) return 'Buat Password Baru';

            return 'Lupa Password';
        },

        /**
         * Mengembalikan form subtitle ke state awal untuk halaman forgot password.
         *
         * @returns {string} Teks reset form subtitle yang telah diformat atau ditentukan.
         */
        resetFormSubtitle() {
            if (this.isVerifyCodeStep) return 'Masukkan kode dari Clerk untuk melanjutkan reset password.';
        ...
        /**
         * Mengembalikan button label ke state awal untuk halaman forgot password.
         *
         * @returns {string} Teks reset button label yang telah diformat atau ditentukan.
         */
        resetButtonLabel() {
            if (this.isVerifyCodeStep) return 'Verifikasi Kode';

            if (this.resetStep === resetPasswordSteps.newPassword) return 'Simpan Password Baru';
    ...
    methods: {
        ...
        /**
         * Memvalidasi and synchronize input email untuk halaman forgot password.
         *
         * @returns {void} Function menerapkan efeknya melalui state komponen atau aplikasi.
         */
        watchInputEmail() {
            if (this.email.trim() !== '') this.errors.email = '';
        },

        /**
         * Memvalidasi and synchronize input code untuk halaman forgot password.
         *
         * @returns {void} Function menerapkan efeknya melalui state komponen atau aplikasi.
         */
        watchInputCode() {
            if (this.code.trim() !== '') this.errors.code = '';
        },

        /**
         * Memvalidasi and synchronize input password untuk halaman forgot password.
         *
         * @returns {void} Function menerapkan efeknya melalui state komponen atau aplikasi.
         */
        watchInputPassword() {
            if (this.password.trim() !== '') this.errors.password = '';
        },

        /**
         * Memvalidasi and synchronize input confirm password untuk halaman forgot password.
         *
         * @returns {void} Function menerapkan efeknya melalui state komponen atau aplikasi.
         */
        watchInputConfirmPassword() {
            if (this.confirmPassword.trim() !== '') this.errors.confirmPassword = '';
        },
        ...
        validateVerifyCodeForm() {
            ...
            if (this.code.trim() === '') this.errors.code = 'input reset code is required';
        ...
        validateNewPasswordForm() {
            ...
            if (this.password.trim().length < 8) this.errors.password = 'password baru minimal 8 karakter';

            if (this.confirmPassword.trim() === '') this.errors.confirmPassword = 'konfirmasi password wajib diisi';
         ...
         *
         * @param {*} [response] Response yang dikembalikan Clerk atau request backend.
         *
         * @returns {*} Nilai yang dihasilkan oleh alur saat ini.
         */
         ...
         *
         * @param {*} resource Resource Clerk yang diperiksa oleh alur authentication.
         *
        ...
        async verifyResetPasswordCode() {
            if (!this.validateVerifyCodeForm()) return;

            this.isProcessingReset = true;

            try {
                const signInResource = this.getActiveSignInResource();
                const verifyResponse = this.assertNoClerkActionError(
        ...
        async submitNewPassword() {
            if (!this.validateNewPasswordForm()) return;

            this.isProcessingReset = true;

            try {
                const signInResource = this.getActiveSignInResource();
                const passwordResponse = this.assertNoClerkActionError(
                    await signInResource.resetPassword({
         ...
         *
         * @param {*} signInAttempt Percobaan login Clerk yang diselesaikan oleh alur reset.
         *
        ...
        async activateCompletedResetSession(signInAttempt) {
            ...
            if (createdSessionId && this.clerkSetActive) await this.clerkSetActive({ session: createdSessionId });

            const runtimeState = await waitForClerkLoaded({ timeout: 1500, interval: 50 });
            const hasActiveSession = Boolean(
                createdSessionId || runtimeState.isSignedIn || getClerkRuntimeState().isSignedIn,
            );
